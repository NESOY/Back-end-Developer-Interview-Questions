...
* 견고함의 원칙(The robustness principle)은 "*당신이 하는 일은 엄하게, 남의 것을 받아들일 때는 너그럽게*"라고 일컬어지는 소프트웨어를 설계할 때 사용되는 일반적인 가이드라인입니다. 이 말은, "*관대한 독자, 신중한 작가가 되어라*"라는 말로써 표현되기도 하죠. 이런 법칙이 합리적인지에 대해서 얘기를 나누어볼 수 있을까요?
* 관심의 분리(Separation of Concerns)는 컴퓨터 프로그램의 부문을 각자의 관심에 따라 나누어 구분짓는다는 설계 원칙입니다. 관심의 분리를 달성하기 위해서는 다양한 메커니즘을 활영할 수 있죠(MVC와 같은 설계에 기반한 객체, 함수, 모듈, 패턴 등을 사용하는 것). 이 주제에 대하여 얘기를 나누어 볼까요?
